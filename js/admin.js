...
(function () {
  ...
  function bindImagePreview(dropSel, fileSel, urlSel, removeSel) {
    ...
    // NOTE: use .oninput (property assignment) instead of addEventListener so that
    // re-calling bindImagePreview() (which happens after every re-render) REPLACES
    // the handler instead of stacking a new one on top of the old ones. The old
    // code used addEventListener here, which caused listeners to multiply every
    // time the field changed (1, then 2, then 4...), making uploads/GitHub-picks
    // fire repeated/duplicate actions ("popup ultapalta" behavior).
    if (urlInput) {
      urlInput.oninput = () => {
